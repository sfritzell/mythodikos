...
for t in file_text:
	...
	# Apply NER to text (comment out if testing other elements, takes time), print all entity results to entity_tupple list
	ner_crawl = ner.tag_ner('greek', input_text=text, output_type=list) # this action returns a list of tuples for each line of text *** FAILS TO IDENTIFY ALL ENTITIES
	for item in ner_crawl:
		if "Entity" in item:
			entity_tuples.append(item)
