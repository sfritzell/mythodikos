...
# ALPHA SORT NAMES
with open(proper_names, "r") as dirty_list: # opens file
	clean_names = strip_length(strip_accents(strip_breathing(dirty_list.read()))) # reads file, removes all diacritic characters 
	...
	del name_list[0 : 17] # removes index 0-16 with "|" character (non-"|" duplicates already in list), "for" loop below only removes some items.  NOT priority; "|" will not match in search.
...
with open(lemma_dict, "r") as dirty_lemmas: # opens file, eval(dirty_lemmas.read()) shows that dirty_lemmas is being read as a string rather than dict
